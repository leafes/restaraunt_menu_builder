...
export default {
  ...
  methods: {
    addMeal(submenuName) {
      if (!this.newMealNames[submenuName] || !this.newMealPrices[submenuName])
        return;
      const submenuToChange = this.subMenus.find(
        (submenu) => submenu.name === submenuName
      );
      submenuToChange.meals.push({
        name: this.newMealNames[submenuName],
        price: Number(this.newMealPrices[submenuName]),
      });
    ...
    addSubMenu() {
      const newSubMenu = {
        name: this.newSubmenuName,
        id: this.subMenus.length,
        meals: [],
      };
    ...
    async saveNewMenu() {
      ...
      this.subMenus.forEach(async (submenu) => {
        const submenuData = await postToDB(
          { title: submenu.name, description: "add meals iter 2" },
          pathForThisMenu
        );
        ...
        submenu.meals.forEach(
          async (meal) =>
            await postToDB(
              { title: meal.name, price: meal.price, description: "bum" },
              pathForThisSubMenu
            )
        );
      });
